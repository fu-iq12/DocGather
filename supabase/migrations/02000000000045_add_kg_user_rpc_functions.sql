...
begin
  ...
  -- (assuming simple paths like "$.first_name")
  
  if p_target_type = 'entity' then
    select data into v_current_data from public.kg_entities_decoded where id = p_target_id;
    -- Path parsing omitted for brevity (in real implementation, split p_json_path and use jsonb_set appropriately)
...
-- -----------------------------------------------------------------------------
-- user_kg_remove_override: Un-confirm a field, letting LLM dictate value again
-- -----------------------------------------------------------------------------
